    ...
    if (nomform!='cancel'){
      ...
      switch(idioma){
        case "ca":
          ...
          break;
        case "es":
          form.setDescription("Este formulario servirá para evaluar las actividades. Primero selecciona a qué compañero evalúas y después, para cada aspecto, elige la descripción que más coincida con su actividad");
          break;
        case "eu":
      ...
      try { 
        ...
        if (rangalumnes.getNumRows()-1===0){
          ...
          switch(idioma){
            case "ca":
              ...
              break;
            case "es":
              Browser.msgBox('Alumnos','¡No has indicado ningún alumno para evaluar!', Browser.Buttons.OK);
              break;
            case "eu":
        ...
        switch(idioma){
          case "ca":
            ...
            break;
          case "es":
            preguntaList.setTitle("Alumno que se evalúa");
            break;
          case "eu":
        ...
        for (i=1;i<rangrubrica.getNumRows()-1;i++){
          ...
          titol = titol.replace(/(?:\r\n|\r|\n)/g, " ");
          var rangtitol = rangrubrica.getCell(i+2,1).setValue(titol);
          //var rangtitol = rangrubrica.getRange(i+2,1,1,1).setValue(titol);
          ...
          if (oblig===ui.Button.YES){
            preguntaGrid.setRequired(true);
          }else{
            preguntaGrid.setRequired(false);
          };
...
function procesFormulari() {
  ...
  var conteMX = confloc.search("_MX");
  var canviloc=0;
  if (conteEN2>0 || conteEN3>0 || conteEN==0 || conteMX>0){   
    var canviloc = 1;
  };
  ...
  //Congelem 2 columnes i 3 files
  fullmitjanes.setFrozenRows(3);
  fullmitjanes.setFrozenColumns(2);
  
  //Guardo els id de cada pregunta del form (per poder sabr les que no es responen)
  var f_preguntes=[];
  var ff_items=form.getItems();
  for (var m=0;m<ff_items.length;m++){
    f_preguntes[m]=ff_items[m].getId();
  };
  ...
  var nom_respon=[]; //Afegim variable per desar qui respon 
  for (var i=0;i<num_resp;i++) {  //Defineixo la matriu com a bidimensional i la poso tot en blanc
     matriu_res[i] = [];
     nom_respon[i]=[]; //Afegim variable per desar qui respon 
     nom_respon[i][0]=0; //Afegim variable per desar qui respon 
     nom_respon[i][1]=0; //Afegim variable per desar qui respon 
    for (var j=0; j<num_col;j++){
       matriu_res[i][j]=0;   
    };
  }; 
 
  var nom_respon_auto=[]; //Afegim variable per desar autoavaluacions
  for (var i=0;i<num_resp;i++) {  //Defineixo la matriu com a bidimensional i la poso tot en blanc
     matriu_res_auto[i] = [];
     nom_respon_auto[i]=[];  
     nom_respon_auto[i][0]=0;
     nom_respon_auto[i][1]=0;
     for (var j=0; j<num_col;j++){
       matriu_res_auto[i][j]=0;   
    };
  }; 
  
  var nom_respon_profe=[]; //Afegim variable per desar resposta profe 
  for (var i=0;i<num_resp;i++) {  //Defineixo la matriu com a bidimensional i la poso tot en blanc
     matriu_res_profe[i] = [];
     nom_respon_profe[i]=[]; 
     nom_respon_profe[i][0]=0;
     nom_respon_profe[i][1]=0;
     for (var j=0; j<num_col;j++){
  ...
  for (var i=0; i < num_resp; i++) {
   var respota_f = re[i];
   nom_respon[s][0]=respota_f.getRespondentEmail(); //Desem qui ha respost 
   var itemResponses = respota_f.getItemResponses();
    ...
    m=0;
    var item_esp=false;
    for (var j=0;j<f_preguntes.length;j++) {
      item_esp=false;
      var itemResponse = itemResponses[m];
      var numpreg=itemResponses[m].getItem().getId()
      if (f_preguntes[j]===numpreg){  //Si la pregunta l'ha respost
        item_esp=true;
      };
      if (j===0) {
        matriu_res[s][j]=itemResponse.getResponse();
        nom_respon[s][1]=itemResponse.getResponse(); //Desem a qui ha avaluat    
        var avaluat="";
        avaluat=nom_respon[s][1];
        if (nom_respon[s][1]===nom_buscat){ //Miro si es AUTO
          matriu_res_auto[x][j]=matriu_res[s][j];
          nom_respon_auto[x][1]=nom_respon[s][1]; //Desem a qui ha avaluat 
          nom_respon_auto[x][0]=nom_respon[s][0]; //Desem qui ha avaluat 
        }else{
          if (profe_trobat===1){ //Miro si es PROFE
            matriu_res_profe[y][j]=matriu_res[s][j];
            nom_respon_profe[y][1]=nom_respon[s][1]; //Desem a qui ha avaluat 
            nom_respon_profe[y][0]=nom_respon[s][0]; //Desem qui ha avaluat 
          };
        }; 
        m++;
      }else{
        if (j===f_preguntes.length-1){
          var comentaris = itemResponse.getResponse();
          Logger.log(j+" "+comentaris);
        }else{
          if (item_esp){
  ...
  //Trobem les persones que han avaluat més d'un cop una persona
  ...
  if (duplicats_profe==='yes'){  //Eliminem les files duplicades 
    ...
    if (fullmitjanes.getRange(i+4,3,1,1).getCell(1,1).getValue()===0){
      ...
      if (resp==1){
        if (canviloc===1){
          ...
          rang_resultats.getCell(i+1,3*alumnes[i].length+3).setFormula("iferror(round(SUMPRODUCT(filter({"+ rangminim +"};isnumber({"+ rangminim +"}));filter({"+ pesos+"};isnumber({"+ rangminim +"})))/sum(filter({"+ pesos +"};isnumber({"+ rangminim +"})))*"+vm+",2)*"+celmaxpunt+"/10;\"-\")");
        }else{
          vm = vm.replace(".", ",");
          rang_resultats.getCell(i+1,3*alumnes[i].length).setFormula("round(min("+rangminim+")*"+vm+";2)*"+celmaxpunt+"/10");
          rang_resultats.getCell(i+1,3*alumnes[i].length+3).setFormula("iferror(round(SUMPRODUCT(filter({"+ rangminim +"};isnumber({"+ rangminim +"}));filter({"+ pesos+"};isnumber({"+ rangminim +"})))/sum(filter({"+ pesos +"};isnumber({"+ rangminim +"})))*"+vm+";2)*"+celmaxpunt+"/10;\"-\")");
    ...
    if (fullmitjanes.getRange(i+4,4,1,1).getCell(1,1).getValue()===0){
      ...
      if (resp_auto==1){
        if (canviloc===1){
          ...
          rang_resultats.getCell(i+1,3*alumnes_auto[i].length+4).setFormula("iferror(round(SUMPRODUCT(filter({"+ rangminim_auto +"};isnumber({"+ rangminim_auto +"}));filter({"+ pesos+"};isnumber({"+ rangminim_auto +"})))/sum(filter({"+ pesos +"};isnumber({"+ rangminim_auto +"})))*"+vm+",2)*"+celmaxpunt+"/10;\"-\")");
        }else{
          vm = vm.replace(".", ",");
          rang_resultats.getCell(i+1,3*alumnes_auto[i].length+1).setFormula("round(min("+rangminim_auto+")*"+vm+";2)*"+celmaxpunt+"/10");
          rang_resultats.getCell(i+1,3*alumnes_auto[i].length+4).setFormula("iferror(round(SUMPRODUCT(filter({"+ rangminim_auto +"};isnumber({"+ rangminim_auto +"}));filter({"+ pesos+"};isnumber({"+ rangminim_auto +"})))/sum(filter({"+ pesos +"};isnumber({"+ rangminim_auto +"})))*"+vm+";2)*"+celmaxpunt+"/10;\"-\")");
    ...
    if (fullmitjanes.getRange(i+4,4,1,1).getCell(1,1).getValue()===0){
      ...
      if (resp_profe==1){
        if (canviloc===1){
          ...
          rang_resultats.getCell(i+1,3*alumnes_profe[i].length+5).setFormula("iferror(round(SUMPRODUCT(filter({"+ rangminim_profe +"};isnumber({"+ rangminim_profe +"}));filter({"+ pesos+"};isnumber({"+ rangminim_profe +"})))/sum(filter({"+ pesos +"};isnumber({"+ rangminim_profe +"})))*"+vm+",2)*"+celmaxpunt+"/10;\"-\")");
        }else{
          vm = vm.replace(".", ",");
          rang_resultats.getCell(i+1,3*alumnes_profe[i].length+2).setFormula("round(min("+rangminim_profe+")*"+vm+";2)*"+celmaxpunt+"/10");
          rang_resultats.getCell(i+1,3*alumnes_profe[i].length+5).setFormula("iferror(round(SUMPRODUCT(filter({"+ rangminim_profe +"};isnumber({"+ rangminim_profe +"}));filter({"+ pesos+"};isnumber({"+ rangminim_profe +"})))/sum(filter({"+ pesos +"};isnumber({"+ rangminim_profe +"})))*"+vm+";2)*"+celmaxpunt+"/10;\"-\")");
...
function nocreaCoRubrics(){
  ...
  switch(idioma){
    case "ca":
      var avis = Browser.msgBox('No crear CoRubrics','Aquest procés suposa que el full de càlcul ja conté les tres pestanyas (Rúbrica, Alumnes i Profes) i, per tant, no els modifica. Vols contiuar?', Browser.Buttons.YES_NO);
      break;
    case "es":
      var avis = Browser.msgBox('No crear CoRubrics','Este proceso supone que la hoja de cálculo ya contiene las tres pestañas (Rúbrica, Alumnos y Profes) y, por tanto, no las modifica. ¿Continuar?', Browser.Buttons.YES_NO);
      break;
    case "eu":
      var avis = Browser.msgBox('Ez CoRubrics sortu','Prozesu honek suposatzen du kalkulu-orriak dagoeneko hiru fitxak dituela (Errubrika, Ikasleak eta Irakasleak) eta, beraz, ez ditu aldatzen. Jarraitzeko?', Browser.Buttons.YES_NO);
      break;
    case "fr":
      var avis = Browser.msgBox('Ne créez CoRubrics','Ce processus suppose que la feuille de calcul contient déjà les trois onglets (Grillé, Élèves et Enseignants) et, par conséquent, ne les modifie pas. Continuer?', Browser.Buttons.YES_NO);
      break;
    default:
      var avis = Browser.msgBox('Don\'t create CoRubrics','This process assumes that the spreadsheet already contains the three tabs (Rubric, Students and Teachers) and, therefore, does not modify them. Continue?', Browser.Buttons.YES_NO);
  }  
  if (avis==='yes'){
    var documentProperties = PropertiesService.getDocumentProperties();
    documentProperties.setProperty('Importacio', "1");
...
function document() {
  ...
  switch(idioma){
    case "ca":
      var html = HtmlService.createHtmlOutputFromFile('docu_cat')
      .setTitle('Opcions del document')
      .setWidth(400);
      break;
    case "es":
      var html = HtmlService.createHtmlOutputFromFile('docu_es')
      .setTitle('Opciones del documento')
      .setWidth(400);
      break;
    case "eu":
      var html = HtmlService.createHtmlOutputFromFile('docu_eu')
      .setTitle('Dokumentuen aukerak')
      .setWidth(400);
      break;
    case "fr":
      var html = HtmlService.createHtmlOutputFromFile('docu_fr')
      .setTitle('Options de document')
      .setWidth(400);
      break;          
    default:
      var html = HtmlService.createHtmlOutputFromFile('index_en')
...
/**
 * Recull les respostes de la barra lateral
 * Crea un document amb els resultats de cada alumne
 * 
 */
function docunotes(formObject) {
  var llibreActual = SpreadsheetApp.getActiveSpreadsheet();
  var properties = PropertiesService.getDocumentProperties();   
  var idioma = properties.getProperty('Idioma');   
  switch(idioma){
  ...
  //Tanquem el menú i posem uptading
  var html = HtmlService.createHtmlOutputFromFile('updating')
      .setTitle('Opcions del document')
      .setWidth(400);
  SpreadsheetApp.getUi() // Or DocumentApp or FormApp.
  .showSidebar(html);
  ...
  switch(idioma){
    ...
    default:
      ...
      var cco ="Comments from colleagues: ";
      var cav ="Student's own comments: ";
      var txt_nom_alumne = "Student";
  } 
  //Crear el document en blanc i moure'l a la carpeta
  var docu = DocumentApp.create(titol); //Crea el document
  var docuid = docu.getId();
  var docuurl = docu.getUrl();
  var llibreActual = SpreadsheetApp.getActiveSpreadsheet();
  //Movem el formualri creat a la carpeta del full de CoRubrics
  var folders = DriveApp.getFileById(llibreActual.getId()).getParents(); //Agafem les carpetes on està el full de CoRubrics
  var folder = folders.next();  //Agafem la primer carpeta. 
  ...
  for (var i=0; i<rangalumnes.getNumRows()-1;i++){
    for (var j=0; j<rangalumnes.getNumColumns()-1;j++){   
      alumnes = r_al[i+1][j];
      if (alumnes!=""){
        var cap = body.appendParagraph(titol);
        cap.setHeading(DocumentApp.ParagraphHeading.HEADING1);
        cap.setAlignment(DocumentApp.HorizontalAlignment.CENTER);
        var txt_alum = body.appendParagraph(txt_nom_alumne+": "+alumnes);
        txt_alum.setHeading(DocumentApp.ParagraphHeading.HEADING3);  
        var taula=body.appendTable();
        //taula.setTextAlignment(DocumentApp.TextAlignment.SUPERSCRIPT);
        var fila=taula.appendTableRow();
        //Definim la rúbrica original i els resultats
        for (var k=1;k<rangrubrica.getNumColumns();k++){
          var cela = fila.appendTableCell(mat_rubrica[0][k-1]);
          cela.setBackgroundColor('#b7b7b7');
        ...
        //Afegeixo la segona fila
        fila = taula.appendTableRow();
        cela = fila.appendTableCell("");
        cela.setBackgroundColor('#b7b7b7');        
        for (var k=2;k<rangrubrica.getNumColumns();k++){
          cela = fila.appendTableCell(mat_rubrica[1][k-1]);
          cela.setVerticalAlignment(DocumentApp.VerticalAlignment.CENTER);
          cela.setBackgroundColor('#b7b7b7');
          var style={};
          style[DocumentApp.Attribute.BOLD] = true;
        ...
        //Afegim la resta de files
        for (var z=2;z<rangrubrica.getNumRows();z++){
          fila = taula.appendTableRow();
          cela = fila.appendTableCell(mat_rubrica[z][0]);
          var style={};
          style[DocumentApp.Attribute.BOLD] = true;
          style[DocumentApp.Attribute.HORIZONTAL_ALIGNMENT] = DocumentApp.HorizontalAlignment.CENTER;
          style[DocumentApp.Attribute.FONT_SIZE] = 8;
          ...
          if (av){
            cela = fila.appendTableCell(valormitjanes[i+3][3*z]);
            cela.setBackgroundColor('#ffe599');
            cela.setVerticalAlignment(DocumentApp.VerticalAlignment.CENTER);
            var style={};
            style[DocumentApp.Attribute.BOLD] = true;
        ...
        //Si hem d'enviar la nota final, l'afegim
        if (notafinal==="yes") {
          taula = body.appendTable();
          fila = taula.appendTableRow();
          fila.appendTableCell("");
          if (co){
            cela = fila.appendTableCell(pc);
            cela.setBackgroundColor('#b7b7b7');
            cela.setTextAlignment(DocumentApp.TextAlignment.NORMAL);
            cela.setVerticalAlignment(DocumentApp.VerticalAlignment.CENTER);
            var style={};
            style[DocumentApp.Attribute.BOLD] = true;
            style[DocumentApp.Attribute.HORIZONTAL_ALIGNMENT] = DocumentApp.HorizontalAlignment.CENTER;
          ...
          if (tipusnotafinal==="yes"){
            if (co){
              cela = fila.appendTableCell(valormitjanes[i+3][rangmitjanes.getNumColumns()-9]);
              cela.setBackgroundColor('#ffe599');
              var style={};
            ...
            if (av){
              cela = fila.appendTableCell(valormitjanes[i+3][rangmitjanes.getNumColumns()-8]);
              cela.setBackgroundColor('#ffe599');
              var style={};
              style[DocumentApp.Attribute.BOLD] = false;
              style[DocumentApp.Attribute.HORIZONTAL_ALIGNMENT] = DocumentApp.HorizontalAlignment.CENTER;
            ...
            if (pf){
              cela = fila.appendTableCell(valormitjanes[i+3][rangmitjanes.getNumColumns()-7]);
              cela.setBackgroundColor('#ffe599');
              var style={};
              style[DocumentApp.Attribute.BOLD] = false;
              style[DocumentApp.Attribute.HORIZONTAL_ALIGNMENT] = DocumentApp.HorizontalAlignment.CENTER;
            ...
            if (co){
              fila.appendTableCell(valormitjanes[i+3][rangmitjanes.getNumColumns()-12]);
              var style={};
              style[DocumentApp.Attribute.BOLD] = false;
              style[DocumentApp.Attribute.HORIZONTAL_ALIGNMENT] = DocumentApp.HorizontalAlignment.CENTER;
              cela.getChild(0).setAttributes(style);
            ...
            if (av){
              fila.appendTableCell(valormitjanes[i+3][rangmitjanes.getNumColumns()-11]);
              var style={};
              style[DocumentApp.Attribute.BOLD] = false;
              style[DocumentApp.Attribute.HORIZONTAL_ALIGNMENT] = DocumentApp.HorizontalAlignment.CENTER;
              cela.getChild(0).setAttributes(style);
            ...
            if (pf){
              fila.appendTableCell(valormitjanes[i+3][rangmitjanes.getNumColumns()-10]);
              var style={};
              style[DocumentApp.Attribute.BOLD] = false;
              style[DocumentApp.Attribute.HORIZONTAL_ALIGNMENT] = DocumentApp.HorizontalAlignment.CENTER;
              cela.getChild(0).setAttributes(style);
        ...
        if (notaglobal==="yes") {
          taula = body.appendTable();
          fila = taula.appendTableRow();
          cela=fila.appendTableCell(nfg);
          cela.setBackgroundColor('#b7b7b7');
          var style={};
          style[DocumentApp.Attribute.BOLD] = true;
          style[DocumentApp.Attribute.HORIZONTAL_ALIGNMENT] = DocumentApp.HorizontalAlignment.CENTER;
          cela.getChild(0).setAttributes(style);
          cela = fila.appendTableCell(valormitjanes[i+3][rangmitjanes.getNumColumns()-6]);
          cela.setBackgroundColor('#93c47d');
          var style={};
          style[DocumentApp.Attribute.BOLD] = false;
          style[DocumentApp.Attribute.HORIZONTAL_ALIGNMENT] = DocumentApp.HorizontalAlignment.CENTER;
          cela.getChild(0).setAttributes(style);
        ...
        //Afegim els comentaris del professor
        if (coment_prof==="yes"){
          body.appendParagraph(cpf+rangmitjanes.getCell(i+4,rangmitjanes.getNumColumns()-2).getValue());
        };
        //Afegim els comentaris dels alumnes
        if (coment_alu==="yes"){
          body.appendParagraph(cco+rangmitjanes.getCell(i+4,rangmitjanes.getNumColumns()-1).getValue());
        };
        //Afegim els comentaris propis
        if (coment_auto==="yes"){
  ...
  //Indiquem que s'ha creat el document
  switch(idioma){
    case "ca":
      var html = HtmlService.createHtmlOutputFromFile('docu_creat_ca')
      .setTitle('Opcions del document')
      .setWidth(400);
      break;
    case "es":
      var html = HtmlService.createHtmlOutputFromFile('docu_creat_es')
      .setTitle('Opciones del documento')
      .setWidth(400);
      break;
    case "eu":
      var html = HtmlService.createHtmlOutputFromFile('docu_creat_eu')
      .setTitle('Dokumentuen aukerak')
      .setWidth(400);
      break;
    case "fr":
      var html = HtmlService.createHtmlOutputFromFile('docu_creat_fr')
      .setTitle('Document options')
      .setWidth(400);
      break; 
    default:
      var html = HtmlService.createHtmlOutputFromFile('docu_creat_en')
      .setTitle('Options de document')
      .setWidth(400);
  } 
  SpreadsheetApp.getUi() // Or DocumentApp or FormApp.
  ...
  //Deso al ScripDb que l'he enviat (si no estic reenviant)
  var documentProperties = PropertiesService.getDocumentProperties();
  documentProperties.setProperty('Document', "1");
...
/*
// Funcìó per crear un primer full
// amb la rúbrica en format Classroom, per poder importar a Classroom
*/

function rubricClass(){
  var properties = PropertiesService.getDocumentProperties(); 
  properties.setProperty('tasca_av_id', "") 
  properties.setProperty('tasca_co_id', "")
  properties.setProperty('tasca_pf_id', "")
  ...
  var nom_full_rc = "RC";
  var sheet = source.getSheetByName(nom_full_rc);
  var destination = SpreadsheetApp.getActiveSpreadsheet();
  var full_rub=sheet.copyTo(destination); 
  full_rub.setName("Google Classroom"); 
  sleep (2000);
  var fulls= destination.getSheetByName("Google Classroom").activate()
