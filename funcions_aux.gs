  ...
  for (i=1;i<res+1;i++){ //Omplo la matriu resposta amb les respostes dels alumnes
    for (j=1; j<col+1;j++){
      ...
      if (j===1){
        ...
        for (var k=0;k<rangrubrica.getNumColumns()-2;k++){
...
function Trobar_alumnes (mat_resp){
  ...
  var alumnes = [];
  var num_resp_preg = [];
  for (var i=0;i<nombrealumnes;i++) {  //Defineixo la matriu com a bidimensional i l'omplo "", escepte la columna 1, que poso 0
    alumnes[i] = [];
    num_resp_preg[i] = [];  //Comptaré el nombre de respostes reals de cada aspectes, per si algun no es valora
    for (var j=0; j<asp+1;j++){
      if (j===1){
        alumnes[i][j]=0;
      }else{
        alumnes[i][j]="";
      };
      num_resp_preg[i][j] = 0; //Compto quantes respostes té cada aspecte
    };
  }; 
  ...
  for (i=0;i<mat_resp.length;i++){
    for (var k=0;k<alumnes.length;k++){
      if (mat_resp[i][0]===alumnes[k][0]){
        ...
        for (var z=2;z<alumnes[k].length;z++){
          if(mat_resp[i][z-1]!="" || mat_resp[i][z-1]===0){
            if (alumnes[k][z]===""){ //Si és la primera puntuació, poso 0 per poder sumar
              alumnes[k][z]=0;
            };              
            alumnes[k][z]=alumnes[k][z]+parseInt(mat_resp[i][z-1]);
            num_resp_preg[k][z]=num_resp_preg[k][z]+1;
          };
        };
      };
    };
  ...
  for (i=0;i<alumnes.length;i++){
    for (j=2;j<alumnes[0].length;j++){
      if (alumnes[i][j]!="" || alumnes[i][j]===0){      
        alumnes[i][j]=alumnes[i][j]/num_resp_preg[i][j];
